"""Method I (LA-IPPO) training on MPE2 simple_tag_v3 (predator-prey) with self-play.
...
Usage:
    ...
    python train_method_i.py --max-steps 200000 --num-envs 8
    python train_method_i.py --max-time 780 --results-key method_i_wall_clock_match
"""
...
def plot_training(
    ...
    ax1.set_title("Method I on MPE2 simple_tag_v3 — Elo vs Pool")
...
def main() -> None:
    ...
    if not results_key:
        results_key = "method_i_wall_clock_match" if max_time > 0 else "method_i"
    ...
    if distill_anneal:
        print(
            f"Distillation annealing: {distill_weight} → {distill_anneal_end} "
            f"over {distill_anneal_steps} distill events"
        )
    ...
    print(
        f"\nStarting Method I self-play for {max_steps} env steps with {num_envs} envs{time_tag}..."
    )
    ...
    pbar = tqdm(total=max_steps, desc="Method I")
    ...
    if results_path:
        ...
        results[results_key] = {
            "description": "Method I self-play with Elo vs rolling opponent pool",
